...
if (!function_exists('trx_addons_cpt_cars_init')) {
	...
	function trx_addons_cpt_cars_init() {
		...
		$TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_CARS_PT] = array(
			...
			"doors" => array(
				"title" => esc_html__("Doors", 'trx_addons'),
				"desc" => wp_kses_data( __('Doors quantity (only digits)', 'trx_addons') ),
				"class" => "trx_addons_column-1_4 trx_addons_new_row",
				"std" => "",
				"type" => "text"
			),
			"passengers" => array(
				"title" => esc_html__("Passengers", 'trx_addons'),
				"desc" => wp_kses_data( __('Possible passengers quantity (only digits)', 'trx_addons') ),
				"class" => "trx_addons_column-1_4",
				"std" => "",
				"type" => "text"
			),
			"luggage" => array(
				"title" => esc_html__("Luggage", 'trx_addons'),
				"desc" => wp_kses_data( __('Possible luggage quantity (only digits)', 'trx_addons') ),
				"class" => "trx_addons_column-1_4",
				"std" => "",
				"type" => "text"
			),
			...
			"agent_type" => array(
				...
				"desc" => wp_kses_data( __("What display in the Agent information block: Agent, Post author or driver data?", 'trx_addons') ),
				"std" => "owner",
				"options" => array(
					"agent" => esc_html__('Agent', 'trx_addons'),
					"author" => esc_html__('Author', 'trx_addons'),
					"owner" => esc_html__('Driver', 'trx_addons')
				),
				"type" => "radio"
			),
			"agent" => array(
			...
			"owner_name" => array(
				...
				"desc" => wp_kses_data( __('Name of the driver', 'trx_addons') ),
				"dependency" => array(
			...
			"owner_phone" => array(
				...
				"desc" => wp_kses_data( __('Phone of the driver', 'trx_addons') ),
				"dependency" => array(
					"agent_type" => array("owner")
				),
				"class" => "trx_addons_column-1_3",
			...
			"owner_email" => array(
				...
				"desc" => wp_kses_data( __('E-mail of the driver', 'trx_addons') ),
				"dependency" => array(
					"agent_type" => array("owner")
				),
				"class" => "trx_addons_column-1_3",
			...
			"owner_skype" => array(
				...
				"desc" => wp_kses_data( __('Skype of the driver', 'trx_addons') ),
				"dependency" => array(
					"agent_type" => array("owner")
				),
				"class" => "trx_addons_column-1_3",
...
if ( !function_exists( 'trx_addons_cpt_cars_save_post_options' ) ) {
	...
	function trx_addons_cpt_cars_save_post_options($options, $post_id, $post_type) {
		if ($post_type == TRX_ADDONS_CPT_CARS_PT) {
			...
			update_post_meta($post_id, 'trx_addons_cars_doors', $options['doors']);
			update_post_meta($post_id, 'trx_addons_cars_passengers', $options['passengers']);
			update_post_meta($post_id, 'trx_addons_cars_luggage', $options['luggage']);
			...
			// Update min and max values of the price, engine_size, mileage, doors, passengers, luggage etc.
...
if ( !function_exists( 'trx_addons_cpt_cars_update_min_max' ) ) {
	function trx_addons_cpt_cars_update_min_max() {
		...
		$rez = $wpdb->get_results( "SELECT min(mileage.meta_value+0) as mileage_min, max(mileage.meta_value+0) as mileage_max,
										 ...
										 min(doors.meta_value+0) as doors_min, max(doors.meta_value+0) as doors_max,
										 min(passengers.meta_value+0) as passengers_min, max(passengers.meta_value+0) as passengers_max,
										 min(luggage.meta_value+0) as luggage_min, max(luggage.meta_value+0) as luggage_max,
									...
									FROM {$wpdb->posts}
										...
										INNER JOIN {$wpdb->postmeta} AS doors ON {$wpdb->posts}.ID = doors.post_id
										INNER JOIN {$wpdb->postmeta} AS passengers ON {$wpdb->posts}.ID = passengers.post_id
										INNER JOIN {$wpdb->postmeta} AS luggage ON {$wpdb->posts}.ID = luggage.post_id
									...
									WHERE 1=1
										...
										AND doors.meta_key='trx_addons_cars_doors'
										AND passengers.meta_key='trx_addons_cars_passengers'
										AND luggage.meta_key='trx_addons_cars_luggage'
...
if ( !function_exists( 'trx_addons_cpt_cars_query_params' ) ) {
	function trx_addons_cpt_cars_query_params($params=array()) {
		...
		if ( ($value = trx_addons_get_value_gp('cars_doors')) != '' )		$params['cars_doors'] = sanitize_text_field($value);
		if ( ($value = trx_addons_get_value_gp('cars_passengers')) != '' )	$params['cars_passengers'] = sanitize_text_field($value);
		if ( ($value = trx_addons_get_value_gp('cars_luggage')) != '' )		$params['cars_luggage'] = sanitize_text_field($value);
...
if ( !function_exists( 'trx_addons_cpt_cars_query_params_to_args' ) ) {
	function trx_addons_cpt_cars_query_params_to_args($params=array(), $new_query=false) {
		...
		if (!empty($params['cars_doors']))
			if ($params['cars_doors']!=trx_addons_cpt_cars_get_min_max('doors_min').','.trx_addons_cpt_cars_get_min_max('doors_max'))
				$args = trx_addons_query_add_meta($args, 'trx_addons_cars_doors', $params['cars_doors']);
		if (!empty($params['cars_passengers']))
			if ($params['cars_passengers']!=trx_addons_cpt_cars_get_min_max('passengers_min').','.trx_addons_cpt_cars_get_min_max('passengers_max'))
				$args = trx_addons_query_add_meta($args, 'trx_addons_cars_passengers', $params['cars_passengers']);
		if (!empty($params['cars_luggage']))
			if ($params['cars_luggage']!=trx_addons_cpt_cars_get_min_max('luggage_min').','.trx_addons_cpt_cars_get_min_max('luggage_max'))
				$args = trx_addons_query_add_meta($args, 'trx_addons_cars_luggage', $params['cars_luggage']);
